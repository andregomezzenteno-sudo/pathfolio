/**
 * PathFolio — proxy de datos de mercado (Cloudflare Worker)
 *
 * Existe por una razón concreta: sin él, la clave de la API de Twelve Data
 * tiene que ir incrustada en el JavaScript del navegador, y por tanto queda
 * a la vista en un repositorio público. Aunque sea una clave de plan gratuito
 * y limitada, una credencial dentro del código fuente es una mala señal en
 * cualquier proyecto que aspire a mirarse con ojos de fintech — y quien la vea
 * puede no llegar nunca a leer la justificación.
 *
...
// 1) Lista blanca de símbolos. Solo se puede pedir lo que la app usa de
//    verdad; cualquier otra cosa se rechaza. La suite de pruebas comprueba
//    que esta lista cubra todos los tickers de allocations.json, así que no
//    puede quedarse atrás si mañana se añade un instrumento.
const ALLOWED_SYMBOLS = new Set([
  'BND', 'BTC/USD', 'DBC', 'DIA', 'EUR/USD', 'GLD', 'GOVT', 'IJR', 'LQD',
  'QQQ', 'SLV', 'SPY', 'TIP', 'VGK', 'VNQ', 'VT', 'VWO', 'XLE', 'XLI',
  'XLK', 'XLV',
]);
...
// 3) Tope de tamaño: la app pide 2500 barras diarias; nada justifica más.
...
export default {
  async fetch(request, env, ctx) {
    ...
    const outputsize = parseInt(url.searchParams.get('outputsize') || '2500', 10);
    ...
    const cacheKey = new Request(`https://cache.pathfolio/${encodeURIComponent(symbol)}/${outputsize}`);
